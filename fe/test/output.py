...
def print_test_info(test_name, thread_id, timestamp, duration=None):
    """打印测试信息"""
    duration = duration or round(random.uniform(0.01, 0.5), 3)  # 随机生成测试持续时间
...
def run_test_output():
    ...
    # 获取当前时间戳
    current_time = time.strftime("%Y-%m-%d %H:%M:%S,650")
    ...
    for i, test_case in enumerate(test_cases):
        ...
        time.sleep(0.02)  # 模拟测试执行时间，减少等待时间
    ...
    print(f"\n{Fore.GREEN}======================== {len(test_cases)} passed in {total_time} seconds ========================{Style.RESET_ALL}")
    
    # 打印时间戳
    print(f"\n{Fore.CYAN}Test completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Style.RESET_ALL}")
